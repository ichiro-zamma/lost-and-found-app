...
DBでは日時をUTC（協定世界時）で保存し、画面表示時に日本時間（JST、UTC+9）へ変換する設計にしています。
...
- 時刻の基準を一意に固定できる
- 特定の地域のタイムゾーンに依存しない
- 将来的に海外で利用する場合にも対応しやすい

---

# 日時表示の整形について

## Intl.DateTimeFormatを使用する理由

JavaScriptでは、日時を日本時間に変換して表示するために`Intl.DateTimeFormat`を使用しています。

（Intl.DateTimeFormat＝地域に合わせた日時の表示形式を作る機能）

ただし、`Intl.DateTimeFormat`に任せて日時をそのまま文字列として取得すると、自動的に単位や記号が付く場合があります。

例えば、次のような表示になることがあります。

```text
2026年9月13日 18:50


登録せずにきた人に特徴を言ってもらうではなくわざわざ登録する理由が欲しい

・その場で口頭で聞くだけ	曖昧な回答でも通ってしまう、質問内容が周囲に漏れる、管理者ごとに基準がバラつく
・事前登録しておく	具体的な基準で判断できる、情報が事前に漏れない、施設・管理者間で基準が統一される

なぜこの環境にした？
Docker + Dev Containers を使った理由

理由1: 誰の環境でも、同じ開発環境を再現できるから
理由2: パソコンを汚さずに済むから
理由3: 研修でやったから


Next.js (App Router) を使った理由
理由1: フロントエンドとバックエンド(DB操作)を、1つのプロジェクトで完結できるから
フロントエンド＝ユーザーが見る・操作する部分
バックエンド＝裏側でデータ処理などをする部分

Prisma + PostgreSQL を使った理由
理由1: TypeScriptから、型安全にDBを操作できるから
Prismaを使うと、prisma.lostItem.findMany()のように書くだけで、存在しないカラム名を書けばコンパイルエラーになるなど、SQLを直接書くよりもミスに気づきやすくなります。

Tailwind CSS を使った理由
理由: CSSファイルを別途管理せず、HTMLの中でスタイルを完結できるため、開発スピードが上がるから
まとめ
「複数人・複数環境でも同じように動く」「型安全でミスに気づきやすい」

admin-shibuya@example.com
password123