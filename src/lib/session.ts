...
export async function createSession(userId: number) {
  ...
  cookieStore.set(COOKIE_NAME, sign(userId), {
    ...
    maxAge: 60 * 60 * 24 ,  
  // Cookieの有効期間を設定 // 60秒 × 60分 × 24時間 = 1日間
  // 「今日、施設に行く」「今日、候補を確認する」という、1日単位での利用シーンに対しての設定
  // 銀行のような高セキュリティなアプリではないが、秘密情報や個人の行動記録(いつどこで何を無くしたか)を扱うため、
  // 必要以上に長くログイン状態を保持せず、Cookieを盗まれた場合の悪用リスクを抑える目的で7日間から1日間に短縮した
  })
}
